...
def filterIngredients():
    conn = mysql.connect()
    cursor = conn.cursor()
    
    ingredientsKey = request.args.get('ingredients')
    
    query = "SELECT * FROM list_skincare where ingredients LIKE '%" + ingredientsKey + "%'"
...
def filterSubcategory():
    ...
    query = "SELECT * FROM list_skincare where ingredients LIKE '%" + ingredientsKey + "%' and subcategory = '" + subcategory + "' ORDER BY reviewed DESC"
    cursor.execute(query)
